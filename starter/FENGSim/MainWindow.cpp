...
void MainWindow::MakeElbowModel(
    ...
    double arc_R, double arc_thickness, double arc_angle // 半圆弧套筒半径、厚度、角度（单位：弧度）
) {
    vtk_widget->Clear();
    ...
    QString filename = m_currentWorkDir + "/rigidbody.info";
    QFile file(filename);
    if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        QTextStream out(&file);
        out << "---\n";
        out << QString("Volume2: (%1,%2,%3)\n").arg(ref2[0]).arg(ref2[1]).arg(ref2[2]);
        out << QString("Volume3: (%1,%2,%3)\n").arg(ref3[0]).arg(ref3[1]).arg(ref3[2]);
        out << QString("Volume4: (%1,%2,%3)\n").arg(ref4[0]).arg(ref4[1]).arg(ref4[2]);
        file.close();
...
void MainWindow::OpenElbowsModule()
{
    if (ui->actionElbows->isChecked())
    {
        ...
        ElbowsDockWidget* elbowsDock = findChild<ElbowsDockWidget*>("ElbowsDock");
           if (!elbowsDock) {
               elbowsDock = new ElbowsDockWidget(this);
        ...
        if (!findChild<ElbowsDockWidget*>()) {
            ElbowsDockWidget* elbowsDock = new ElbowsDockWidget(this);
            elbowsDock->setObjectName("ElbowsDock");
            addDockWidget(Qt::LeftDockWidgetArea, elbowsDock);
        }
        // 否则就显示它
